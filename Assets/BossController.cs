    ...
    void Update()
    {
        ...
        else if (!animationLocked)
        {
            ...
            if (aggroed)
            {
                ...
                else if (Vector2.Distance(playerPos.position, rigidBody.position) <= attackRangedRange && Vector2.Distance(playerPos.position, rigidBody.position) >= attackRange && projectileTime + delaybtwProjectile < Time.time)
                {
                ...
                else
                {
                    ...
                    {
                        if (playerPos.position.x - rigidBody.position.x > 0.3f)
                        {
                            ChangeAnimationState(RUN);
                            controller.Move(moveSpeed * Time.deltaTime);
                        }
                        else if (rigidBody.position.x - playerPos.position.x > 0.3f)
                        {
                            ChangeAnimationState(RUN);
                            controller.Move(-moveSpeed * Time.deltaTime);
                        }
