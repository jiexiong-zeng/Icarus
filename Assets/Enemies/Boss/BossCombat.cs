...
public class BossCombat : MonoBehaviour
{
    ...
    public void Attack(float delay, int attackDamage, float knockback)
    {
        if (attackPoint == null)
        {
            return;
        }
        StartCoroutine(AttackRoutine(delay, attackDamage, knockback));
    }

    public IEnumerator AttackRoutine(float delay, int attackDamage, float knockback)
    {
        ...
        if (!dazed)
        {
            foreach (Collider2D player in hitPlayer)
            {
                ...
                hitVector.y += 0.01f;
                player.attachedRigidbody.AddForce(hitVector * 1000 * knockback);
    ...
    public IEnumerator HomingRoutine(float delay)
    {
        ...
        if (!dazed)
        {
            Instantiate(HomingProjectile, GameObject.FindGameObjectWithTag("Player").transform.position, Quaternion.identity);
        }
