...
public class GolemController : MonoBehaviour
{
    ...
    const string IDLE = "Golem_Idle";
    const string RUN = "Golem_Walk";
    const string ATTACK = "Golem_Attack";
    const string HURT = "Golem_Hurt";
    const string DEATH = "Golem_Death";
    const string AGGRO = "Golem_Aggro";
    ...
    void GolemAttack()
    {
        if (Time.time > attackTime + 1f && currentState == ATTACK && Time.time < attackTime + 1.4f)
        {
            controller.Move(moveSpeed* 7* transform.localScale.x * Time.deltaTime);
            //transform.position += new Vector3(transform.localScale.x * 0.01f, 0, 0);
        }
    }
